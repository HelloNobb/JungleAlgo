...
def mypage():
    ...
    # 오늘의 목표 가져오기
    today_goal = res.get('today_goal', {}) if res else {}
    goal_amount = None
    if today_goal:
        # 오늘 날짜와 목표 날짜가 같은지 확인
        kst = pytz.timezone('Asia/Seoul')
        today_str = datetime.now(kst).strftime("%Y-%m-%d")
        if today_goal.get('date') == today_str:
            goal_amount = today_goal.get('goal_amount')
...
def create_review():
    try:
        ...
        # 필수 필드 검증
        if not review_text or not problem_id or not star:
            return jsonify({"result": "fail", "message": "모든 필드를 입력해주세요."}), 400
        
        # 중복 리뷰 체크 (같은 사용자가 같은 문제에 대해 리뷰를 이미 작성했는지)
        existing_review = reviews_collection.find_one({
            "backjun_id": current_user,
            "problem_id": int(problem_id)
        })
        
        if existing_review:
            return jsonify({"result": "fail", "message": "이미 해당 문제에 대한 리뷰를 작성하셨습니다."}), 400
        ...
        if result.inserted_id:
            return jsonify({"result": "success", "message": "리뷰가 성공적으로 등록되었습니다."})
        else:
            return jsonify({"result": "fail", "message": "리뷰 등록에 실패했습니다."}), 500
            
    except Exception as e:
        return jsonify({"result": "fail", "message": f"서버 오류: {str(e)}"}), 500
    ...
    try:
        ...
        for review in reviews:
            review_list.append({
                ...
                "backjun_id": review.get("backjun_id"),
                "review_text": review.get("review_text"),
                "difficulty": "Unknown",  # 필요시 문제 난이도 정보 추가
...
def update_review():
    try:
        ...
        edited_star = request.form.get('edited_star')
        
        # 필수 필드 검증
        if not problem_id or not edited_text or not edited_star:
            return jsonify({"result": "fail", "message": "모든 필드를 입력해주세요."}), 400
        ...
        if not existing_review:
            return jsonify({"result": "fail", "message": "수정할 리뷰를 찾을 수 없습니다."}), 404
        
        # 리뷰 업데이트
        result = reviews_collection.update_one(
            {"backjun_id": current_user, "problem_id": int(problem_id)},
            {
                "$set": {
                    "review_text": edited_text,
                    "star": int(edited_star),
                    "review_date": datetime.now().strftime("%Y-%m-%d %H:%M"),
        ...
        if result.modified_count > 0:
            return jsonify({"result": "success", "message": "리뷰가 성공적으로 수정되었습니다."})
        else:
            return jsonify({"result": "fail", "message": "리뷰 수정에 실패했습니다."}), 500
...
def delete_review():
    try:
        ...
        if not problem_id:
            return jsonify({"result": "fail", "message": "문제 ID가 필요합니다."}), 400
        ...
        if result.deleted_count > 0:
            ...
            return jsonify({"result": "fail", "message": "리뷰 삭제에 실패했습니다."}), 500
